...
options.register(
    "fillPU", True, VarParsing.multiplicity.singleton, VarParsing.varType.bool, "Fill PU"
)
### Options for upgrade studies
# Change hits requirements
options.register(
    "changeMinNumberOfHits",
    False,
    VarParsing.multiplicity.singleton,
    VarParsing.varType.bool,
...
options.register(
    "skipEvents", 0, VarParsing.multiplicity.singleton, VarParsing.varType.int, "skip N events"
)
options.register(
    "runTiming",
    False,
...
options.register(
    "numThreads", 1, VarParsing.multiplicity.singleton, VarParsing.varType.int, "number of threads"
)
options.register(
    "numStreams", 1, VarParsing.multiplicity.singleton, VarParsing.varType.int, "number of streams"
)
options.register(
    "reco",
    "HLT_GRun",
...
if options.defaults:
    ...
    try:
        defaults = import_module(
            "RecoBTag.PerformanceMeasurements.defaults_HLT.%s" % options.defaults
        )
    except ImportError:
        raise ValueError(
            "The default settings named %s.py are not present in PerformanceMeasurements/python/defaults_HLT/"
            % options.defaults
        )
    if not hasattr(defaults, "common") or not isinstance(defaults.common, dict):
        raise RuntimeError(
            "the default file %s.py does not contain a dictionary named common" % options.defaults
        )
    items = defaults.common.items()
    if hasattr(defaults, "data") and options.runOnData:
        if not isinstance(defaults.data, dict):
            raise RuntimeError(
                'the default file %s.py contains an object called "data" which is not a dictionary'
                % options.defaults
            )
    ...
    if hasattr(defaults, "mc") and not options.runOnData:
        if not isinstance(defaults.mc, dict):
            raise RuntimeError(
                'the default file %s.py contains an object called "mc" which is not a dictionary'
                % options.defaults
            )
    ...
    for key, value in items:
        if key not in options._beenSet:
            raise ValueError(
                "The key set by the defaults: %s does not exist among the cfg options!" % key
            )
        elif not options._beenSet[key]:
            if key == "inputFiles" and options.inputFiles:
                continue  # skip input files that for some reason are never considered set
...
for switch in options_to_change:
    if switch in ["runTagVariablesSubJets", "runCSVTagVariablesSubJets"]:
        ...
        raise ValueError(
            "The option set by the variables: %s does not exist among the cfg options!" % switch
        )
...
if options.runOnData:
    options.isReHLT = False
if options.isReHLT:
    trigresults = trigresults + "2"
...
###
if options.runOnData:
    from RecoBTag.PerformanceMeasurements.Configs.HLT_dev_CMSSW_12_3_0_GRun_configDump_Data import (
        cms,
        process,
    )
else:
...
if options.reco == "HLT_GRun" or options.reco == "HLT_GRun_oldJECs":
    ...
    if hasattr(process, "hltDeepCombinedSecondaryVertexBJetTagsCalo"):
        process.hltDeepCombinedSecondaryVertexBJetTagsCalo.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
    if hasattr(process, "hltDeepCombinedSecondaryVertexBJetTagsPF"):
        process.hltDeepCombinedSecondaryVertexBJetTagsPF.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
    if hasattr(process, "hltDeepCombinedSecondaryVertexBJetTagsPFAK8"):
        process.hltDeepCombinedSecondaryVertexBJetTagsPFAK8.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
    if hasattr(process, "hltDeepCombinedSecondaryVertexBPFPuppiPatJetTags"):
        process.hltDeepCombinedSecondaryVertexBPFPuppiPatJetTags.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
    if hasattr(process, "hltDeepCombinedSecondaryVertexBPFPatJetTags"):
        process.hltDeepCombinedSecondaryVertexBPFPatJetTags.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
    if hasattr(process, "hltDeepCombinedSecondaryVertexCaloPatBJetTags"):
        process.hltDeepCombinedSecondaryVertexCaloPatBJetTags.NNConfig = cms.FileInPath(
            "RecoBTag/Combined/data/model_no_offset_DeepCSV.json"
        )
else:
...
for el in list(els):
    if (
        (type(els[el]) == cms.OutputModule)
        or (type(els[el]) == cms.EndPath)
        or el == "PrescaleService"
        or el == "datasets"
        or el == "streams"
    ):
...
if options.runTiming:
    ...
    process.options.numberOfThreads = cms.untracked.uint32(
        options.numThreads if (options.numThreads > 1) else 1
    )
    process.options.numberOfStreams = cms.untracked.uint32(
        options.numStreams if (options.numStreams > 1) else 1
    )
    ...
    process.hltTrigReport = cms.EDAnalyzer(
        "HLTrigReport",
...
if not options.runOnData:
    process.prunedGenParticlesBoost = cms.EDProducer(
        "GenParticlePruner",
...
process.btagana.useTrackHistory = (
    options.useTrackHistory
)  ## Can only be used with GEN-SIM-RECODEBUG files
process.btagana.produceJetTrackTruthTree = (
    options.useTrackHistory
)  ## can only be used with GEN-SIM-RECODEBUG files and when useTrackHistory is True
process.btagana.produceAllTrackTree = (
    options.produceAllTrackTree
)  ## True if you want to run info for all tracks : for commissioning studies
# ------------------
process.btagana.runTagVariables = (
    options.runTagVariables
)  ## True if you want to run TagInfo TaggingVariables
process.btagana.runCSVTagVariables = (
    options.runCSVTagVariables
)  ## True if you want to run CSV TaggingVariables
process.btagana.runCSVTagTrackVariables = (
    options.runCSVTagTrackVariables
)  ## True if you want to run CSV Tagging Track Variables
process.btagana.runDeepFlavourTagVariables = options.runDeepFlavourTagVariables
process.btagana.tofPIDColl = cms.InputTag("generalTracksTOFPIDProducerFoooooooooo:t0Src")
...
if monitorTracks:
    ...
    process.TrackHistograms_hltPixelTracksCleanForBTag = TrackHistogrammer.clone(
        src="hltPixelTracksCleanForBTag"
    )
    process.TrackHistograms_hltMergedTracks = TrackHistogrammer.clone(src="hltMergedTracks")
    process.TrackHistograms_hltPixelTracksForBTag = TrackHistogrammer.clone(
        src="hltPixelTracksForBTag"
    )
    process.TrackHistograms_hltPixelTracksZetaClean = TrackHistogrammer.clone(
        src="hltPixelTracksZetaClean"
    )
...
if monitorVertices:
    ...
    process.VertexHistograms_hltTrimmedPixelVertices = VertexHistogrammer.clone(
        src="hltTrimmedPixelVertices"
    )
    process.VertexHistograms_hltVerticesPF = VertexHistogrammer.clone(src="hltVerticesPF")
    process.VertexHistograms_hltVerticesPFFilter = VertexHistogrammer.clone(
        src="hltVerticesPFFilter"
    )
    process.VertexHistograms_hltVerticesL3FilterROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesL3FilterROIForBTag"
    )
    process.VertexHistograms_hltVerticesL3SelectorROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesL3SelectorROIForBTag"
    )
    process.VertexHistograms_hltVerticesL3ROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesL3ROIForBTag"
    )
    process.VertexHistograms_hltVerticesPFROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesPFROIForBTag"
    )
    process.VertexHistograms_hltVerticesPFSelectorROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesPFSelectorROIForBTag"
    )
    process.VertexHistograms_hltVerticesPFFilterROIForBTag = VertexHistogrammer.clone(
        src="hltVerticesPFFilterROIForBTag"
    )
    # #
...
if options.runTiming:
    # timing test
    from HLTrigger.Timer.FastTimer import (
    ...
    process.FastTimerService.dqmTimeRange = 1000.0  # ms
    process.FastTimerService.dqmTimeResolution = 5.0  # ms
    process.FastTimerService.dqmPathTimeRange = 100.0  # ms
    process.FastTimerService.dqmPathTimeResolution = 0.5  # ms
    process.FastTimerService.dqmModuleTimeRange = 40.0  # ms
    ...
    process.dqmOutput = cms.OutputModule(
        "DQMRootOutputModule", fileName=cms.untracked.string("DQM.root")
    )
    ...
    process.FastTimerService.dqmTimeRange = 20000.0
    process.FastTimerService.dqmTimeResolution = 10.0
    process.FastTimerService.dqmPathTimeRange = 10000.0
    process.FastTimerService.dqmPathTimeResolution = 5.0
    process.FastTimerService.dqmModuleTimeRange = 1000.0
    process.FastTimerService.dqmModuleTimeResolution = 1.0
