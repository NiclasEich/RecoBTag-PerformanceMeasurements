// -*- C++ -*-
//
// Package:    HLTTwoFileBTagAnalyzerT
// Class:      HLTTwoFileBTagAnalyzerT
//
/**\class HLTTwoFileBTagAnalyzer HLTTwoFileBTagAnalyzer.cc RecoBTag/PerformanceMeasurements/plugins/HLTTwoFileBTagAnalyzer.cc
...
typedef edm::View<pat::MET> PatMetCollection;
typedef edm::View<pat::Muon> PatMuonCollection;
typedef edm::View<pat::Electron> PatElectronCollection;
typedef edm::View<pat::Photon> PatPhotonCollection;
typedef edm::View<reco::Vertex> PatVertexCollection;
...
class HLTTwoFileBTagAnalyzerT : public edm::one::EDAnalyzer<edm::one::SharedResources>
// class HLTTwoFileBTagAnalyzerT : public edm::EDAnalyzer
{
public:
  explicit HLTTwoFileBTagAnalyzerT(const edm::ParameterSet&);
  ~HLTTwoFileBTagAnalyzerT();
...
private:
  ...
  edm::EDGetTokenT<edm::View<pat::Muon>> Reco_MuonCollectionTag_;
  edm::EDGetTokenT<edm::View<pat::Electron>> Reco_ElectronCollectionTag_;
  // edm::EDGetTokenT<edm::ValueMap<bool>> Reco_ElectronIDTag_;
  std::string Reco_ElectronIDTag_;
  edm::EDGetTokenT<edm::View<pat::MET>> Reco_MetCollectionTag_;
  edm::EDGetTokenT<edm::View<pat::Photon>> Reco_PhotonCollectionTag_;
  edm::EDGetTokenT<edm::View<pat::Jet>> Reco_JetCollectionTag_;
  edm::EDGetTokenT<edm::View<reco::Vertex>> Reco_VerticesCollectionTag_;
  // std::vector<edm::EDGetTokenT<reco::JetTagCollection> > Reco_DeepCSVCollectionTag_;
  std::vector<std::string > Reco_DeepCSVCollectionTag_;
  ...
  edm::View<pat::Muon> reco_muons ;
  edm::View<pat::Electron> reco_electrons ;
  edm::View<pat::Photon> reco_photons ;
  edm::View<pat::MET> reco_met ;
  edm::View<pat::Jet> reco_jets ;
  edm::View<reco::Vertex> reco_vertices ;
...
template<typename IPTI,typename VTX>
HLTTwoFileBTagAnalyzerT<IPTI,VTX>::HLTTwoFileBTagAnalyzerT(const edm::ParameterSet& iConfig):
...
//   variableParser.printVariables(variableSet);
  ...
  Reco_JetCollectionTag_ = consumes<PatJetCollection>(iConfig.getParameter<edm::InputTag>("reco_Jets"));
  Reco_MetCollectionTag_ = consumes<PatMetCollection>(iConfig.getParameter<edm::InputTag>("reco_met"));
  Reco_PhotonCollectionTag_ = consumes<PatPhotonCollection>(iConfig.getParameter<edm::InputTag>("reco_photons"));
  Reco_MuonCollectionTag_ = consumes<PatMuonCollection>(iConfig.getParameter<edm::InputTag>("reco_muonCollectionName"));
  Reco_ElectronCollectionTag_ = consumes<PatElectronCollection>(iConfig.getParameter<edm::InputTag>("reco_electronCollectionName"));
  // Reco_ElectronIDTag_ = consumes<edm::ValueMap<bool>>(iConfig.getParameter<edm::InputTag>("reco_electronID"));
  Reco_ElectronIDTag_ = iConfig.getParameter<std::string>("reco_electronID");
  Reco_VerticesCollectionTag_ = consumes<PatVertexCollection>(iConfig.getParameter<edm::InputTag>("reco_vertices"));
  // Reco_DeepCSVCollectionTag_ = consumes<PatJetCollection>(iConfig.getParameter<edm::InputTag>("reco_btagAlgosDeepCSV"));
  // std::vector<edm::EDGetTokenT<reco::JetTagCollection> > Reco_DeepCSVCollectionTag_;
  Reco_DeepCSVCollectionTag_ = iConfig.getParameter<std::vector<std::string>>("reco_btagAlgosDeepCSV");
  // Reco_DeepJetCollectionTag_ = consumes<PatJetCollection>(iConfig.getParameter<edm::InputTag>("reco_Jets"));
...
template<typename IPTI,typename VTX>
HLTTwoFileBTagAnalyzerT<IPTI,VTX>::~HLTTwoFileBTagAnalyzerT()
{
}
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  ...
  // BTAG mu efficiencies

  edm::Handle<edm::View<pat::Muon> >  reco_muonsHandle;
  iEvent.getByToken(Reco_MuonCollectionTag_,reco_muonsHandle);
  reco_muons = *reco_muonsHandle;
  edm::Handle<edm::View<pat::Electron> >  reco_electronsHandle;
  iEvent.getByToken(Reco_ElectronCollectionTag_,reco_electronsHandle);
  reco_electrons = *reco_electronsHandle;
  // edm::Handle<edm::ValueMap<bool> >  reco_electronsIDHandle;
  // iEvent.getByToken(Reco_ElectronIDTag_,reco_electronsIDHandle);
  // reco_electrons = *reco_electronsHandle;
  edm::Handle<edm::View<pat::Photon> >  reco_photonsHandle;
  iEvent.getByToken(Reco_PhotonCollectionTag_,reco_photonsHandle);
  reco_photons = *reco_photonsHandle;
  edm::Handle<edm::View<pat::MET> >  reco_metHandle;
  iEvent.getByToken(Reco_MetCollectionTag_,reco_metHandle);
  reco_met = *reco_metHandle;
  edm::Handle<edm::View<pat::Jet> >  reco_jetsHandle;
  iEvent.getByToken(Reco_JetCollectionTag_,reco_jetsHandle);
  reco_jets = *reco_jetsHandle;
  edm::Handle<edm::View<reco::Vertex> >  reco_verticesHandle;
  iEvent.getByToken(Reco_VerticesCollectionTag_,reco_verticesHandle);
  reco_vertices = *reco_verticesHandle;

  StringCutObjectSelector<pat::MET, true> metSelection_("pt > 0");
  StringCutObjectSelector<pat::Jet, true> jetSelection_("");
  StringCutObjectSelector<pat::Electron, true> eleSelection_("");
  StringCutObjectSelector<pat::Muon, true> muoSelection_("");
  StringCutObjectSelector<pat::Photon, true> phoSelection_("");
  StringCutObjectSelector<pat::Jet, true> HTdefinition_("");
  StringCutObjectSelector<pat::Jet, true> MHTdefinition_("");

  StringCutObjectSelector<reco::Vertex, true> vtxSelection_("!isFake");

  StringCutObjectSelector<reco::Jet, true> bjetSelection_("");

  bool applyLeptonPVcuts_ = false;
  uint nelectrons_ = 0;
  uint nmuons_ = 1;
  uint nphotons_ = 0;
  uint njets_ = 1;
  ...
  lepPVcuts_.dxy = 9999.;
  lepPVcuts_.dz = 9999.;
  float invMassLowercut_ = 0.;
  float invMassUppercut_ = 9999.;
  bool opsign_ = 0.;
  float leptJetDeltaRmin_ = 0.4;
  float bJetDeltaEtaMax_ = 9999.;
  float bJetMuDeltaRmax_ = 0.4;
  float HTcut_ = 0.;
  float MHTcut_ = 0.;
  float invMassCutInAllMuPairs_ = 0.;
  float workingpoint_ = 0.2;

  //Primary Vertex selection
    const reco::Vertex* reco_pv = nullptr;
    ...
    double met_pt(-99.);
    double met_phi(-99.);
    const pat::MET& pfmet = reco_met.front();
    ...
    // edm::Handle<edm::View<reco::GsfElectron> > eleHandle;
      ...
      if (!reco_electronsHandle.isValid() && nelectrons_ > 0) {
      ...
      // if (!reco_electronsIDHandle.isValid() && nelectrons_ > 0) {
      //   edm::LogWarning("TopMonitor") << "Electron ID handle not valid \n";
      //   return;
      // }

      std::vector<pat::Electron> reco_electrons;
      if (nelectrons_ > 0) {
        if (reco_electronsHandle->size() < nelectrons_) {
          return;
        }

        for (size_t index = 0; index < reco_electronsHandle->size(); index++) {
          const auto e = reco_electronsHandle->at(index);
          const auto el = reco_electronsHandle->ptrAt(index);

          // bool pass_id = (*reco_electronsIDHandle)[el];
          bool pass_id = e.electronID(Reco_ElectronIDTag_);

          if (eleSelection_(e) && pass_id) {
            reco_electrons.push_back(e);
          }
        ...
        if (reco_electrons.size() < nelectrons_) {
          return;
        }
      }
      ...
      if (!reco_muonsHandle.isValid() && nmuons_ > 0) {
        edm::LogWarning("TopMonitor") << "Muon handle not valid \n";
        return;
      }

      if (reco_muonsHandle->size() < nmuons_) {
        return;
      }

      std::vector<reco::Muon> reco_muons;
      if (nmuons_ > 0) {
        for (auto const& m : *reco_muonsHandle) {
          if (muoSelection_(m)) {
            reco_muons.push_back(m);
          }
      ...
      double mll(-2);
      if (nmuons_ > 1) {
        mll = (reco_muons[0].p4() + reco_muons[1].p4()).M();

        if ((invMassUppercut_ > -1) && (invMassLowercut_ > -1) && ((mll > invMassUppercut_) || (mll < invMassLowercut_))) {
          return;
        }
        if (opsign_ && (reco_muons[0].charge() == reco_muons[1].charge())) {
          return;
        }
      }
      ...
      if (!reco_photonsHandle.isValid()) {
        edm::LogWarning("TopMonitor") << "Photon handle not valid \n";
        return;
      }
      if (reco_photonsHandle->size() < nphotons_) {
        return;
      }

      std::vector<pat::Photon> photons;
      for (auto const& p : *reco_photonsHandle) {
        if (phoSelection_(p)) {
          photons.push_back(p);
        }
      }
      ...
      // edm::Handle<reco::PFJetCollection> jetHandle;
        // iEvent.getByToken(jetToken_, reco_jetsHandle);
        if (!reco_jetsHandle.isValid() && njets_ > 0) {
          edm::LogWarning("TopMonitor") << "Jet handle not valid \n";
          return;
        }
        std::vector<pat::Jet> reco_jets;
        if (njets_ > 0) {
          if (reco_jetsHandle->size() < njets_)
            return;
          for (auto const& j : *reco_jetsHandle) {
            if (HTdefinition_(j)) {
              eventHT += j.pt();
            }
            if (MHTdefinition_(j)) {
            ...
            if (jetSelection_(j)) {
              ...
              if (isJetOverlappedWithLepton)
                continue;
              if (nelectrons_ > 0) {
                for (auto const& e : reco_electrons) {
              ...
              if (isJetOverlappedWithLepton)
                continue;
              reco_jets.push_back(j);
            }
          }
          if (reco_jets.size() < njets_)
            return;
        }
        ...
        if (nmuons_ > 2) {
          double mumu_mass;
          for (unsigned int idx = 0; idx < reco_muons.size(); idx++) {
            for (unsigned int idx2 = idx + 1; idx2 < reco_muons.size(); idx2++) {
              //compute inv mass of two different leptons
              mumu_mass = (reco_muons[idx2].p4() + reco_muons[idx2].p4()).M();
        ...
        // JetTagMap bjets;
        // std::vector<pat::Jet>  reco_bjets;
        // std::map<pat::Jet,float>  reco_bjets;
        std::vector<pat::Jet>  reco_bjets;
        std::vector<float>  reco_bjets_values;

          if (nbjets_ > 0) {
            // map of Jet,btagValues (for all jets passing bJetSelection_)
            //  - btagValue of each jet is calculated as sum of values from InputTags in Reco_DeepCSVCollectionTag_
            // JetTagMap allJetBTagVals;
            // std::vector<float>  allJetBTagVals;

            for (auto const& j : *reco_jetsHandle) {
                float btagvalSum = 0.;
                for (const auto& reco_jetTagToken : Reco_DeepCSVCollectionTag_) {
              // edm::Handle<reco::JetTagCollection> bjetHandle;
              // iEvent.getByToken(reco_jetTagToken, bjetHandle);
              ...
              // for (auto const& j : *reco_jetsHandle) {
                ...
                if (not bjetSelection_(j)) {
                  continue;
                }

                // const auto btagVal = i_jetTag.second;
                const auto btagVal = j.bDiscriminator(reco_jetTagToken);

                if (not std::isfinite(btagVal)) {
                  continue;
                }
                btagvalSum = btagvalSum + btagVal;
                // allJetBTagVals.push_back(btagvalSum);
                ...
                // }
                if(btagvalSum>workingpoint_){
                    reco_bjets.push_back(j);
                    reco_bjets_values.push_back(btagvalSum);
                }
              }
            }
            ...
            //   }
            //
            //   reco_bjets.insert(JetTagMap::value_type(jetBTagVal.first, jetBTagVal.second));
            // }

            if (reco_bjets.size() < nbjets_) {
              return;
            }
          }
          ...
          if (nbjets_ > 1) {
            // double deltaEta = std::abs(reco_bjets.begin()->first->eta() - (++reco_bjets.begin())->first->eta());
            double deltaEta = std::abs(reco_bjets.at(0).eta() - reco_bjets.at(1).eta());
          ...
          if ((nbjets_ > 0) && (nmuons_ > 0)) {
            ...
            for (auto const& bjet : reco_bjets) {
              for (auto const& mu : reco_muons) {
                double dR = deltaR(bjet, mu);
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::processTrig(const edm::Handle<edm::TriggerResults>& trigRes, const std::vector<std::string>& triggerList)
{
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::processJets(const edm::Handle<PatJetCollection>& jetsColl, const edm::Handle<PatJetCollection>& jetsColl2,
    const edm::Event& iEvent, const edm::EventSetup& iSetup, const int iJetColl,
...
// end HLTTwoFileBTagAnalyzerT:: processJets
...
template<typename IPTI,typename VTX>
template<typename SVColl>
// void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::processCaloJets(const edm::Handle<JetColl>& jetsColl, const edm::EDGetTokenT<ShallowTagCollection>& jetTagsCollToken, const edm::EDGetTokenT<std::vector<SVColl> >& svCollToken,
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::processCaloJets(const edm::Handle<PatJetCollection>& jetsColl, const edm::EDGetTokenT<ShallowTagCollection>& jetTagsCollToken, const edm::EDGetTokenT<std::vector<SVColl> >& svCollToken,
						// const edm::EDGetTokenT<HLTBTagValue>& CSVToken,
                        const edm::EDGetTokenT<HLTBTagValue>& deepCSVToken,
						const edm::Event& iEvent, const edm::EventSetup& iSetup, const int iJetColl)
{
...
template<typename IPTI,typename VTX>
float HLTTwoFileBTagAnalyzerT<IPTI,VTX>::calculPtRel(const reco::Track& theMuon, const pat::Jet& theJet )
{
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::setTracksPVBase( const reco::TrackRef & trackRef, const edm::Handle<reco::VertexCollection> & pvHandle, int & iPV, float & PVweight )
{
  iPV = -1;
  PVweight = 0.;
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::setTracksPVBaseCalo( const reco::Track & track, const edm::Handle<reco::VertexCollection> & pvHandle, int & iPV, float & PVweight )
{
  iPV = -1;
  PVweight = 0.;
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::beginJob() {
  //cat 0
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::endJob() {
}

template<typename IPTI,typename VTX>
std::vector< float > HLTTwoFileBTagAnalyzerT<IPTI,VTX>::getTrackProbabilies(std::vector< float > v, const int ipType){
...
template<typename IPTI,typename VTX>
double HLTTwoFileBTagAnalyzerT<IPTI,VTX>::calculProbability(std::vector< float > v) {
...
template<typename IPTI,typename VTX>
bool HLTTwoFileBTagAnalyzerT<IPTI,VTX>::findCat(const reco::Track* track, CategoryFinder& d) {
  //int numcat=-1;
...
template<typename IPTI,typename VTX>
const edm::Ptr<reco::Muon> HLTTwoFileBTagAnalyzerT<IPTI,VTX>::matchMuon(const edm::Ptr<reco::Candidate>& theMuon, const edm::View<reco::Muon>& muons ){
...
template<typename IPTI,typename VTX>
std::vector<simPrimaryVertex> HLTTwoFileBTagAnalyzerT<IPTI,VTX>::getSimPVs(const edm::Handle<edm::HepMCProduct>& evtMC){
...
template<typename IPTI,typename VTX>
int HLTTwoFileBTagAnalyzerT<IPTI,VTX>::isFromGSP(const reco::Candidate* c)
{
  int isFromGSP = 0;
...
template<typename IPTI,typename VTX>
bool HLTTwoFileBTagAnalyzerT<IPTI,VTX>::isHardProcess(const int status)
{
  // if Pythia8
...
// -------------------------------------------------------------------------
template<typename IPTI,typename VTX>
bool HLTTwoFileBTagAnalyzerT<IPTI,VTX>::NameCompatible(const std::string& pattern, const std::string& name)
{
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::matchGroomedJets(const edm::Handle<PatJetCollection>& jets,
...
// -------------- etaRelToTauAxis ----------------
template<>
void HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::etaRelToTauAxis(const Vertex & vertex, const fastjet::PseudoJet & tauAxis, std::vector<float> & tau_trackEtaRel)
{
...
template<>
void HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::etaRelToTauAxis(const Vertex & vertex, const fastjet::PseudoJet & tauAxis, std::vector<float> & tau_trackEtaRel)
{
  math::XYZVector direction(tauAxis.px(), tauAxis.py(), tauAxis.pz());
...
// -------------- toIPTagInfo ----------------
template<>
const HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::IPTagInfo *
HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::toIPTagInfo(const pat::Jet & jet, const std::string & tagInfos)
{
  return jet.tagInfoTrackIP(tagInfos.c_str());
}

// template<>
// const HLTTwoFileBTagAnalyzerT<reco::JTATagInfo,reco::Vertex>::IPTagInfo *
// HLTTwoFileBTagAnalyzerT<reco::JTATagInfo,reco::Vertex>::toIPTagInfo(const pat::Jet & jet, const std::string & tagInfos)
// {
//   return jet.tagInfoJTA(tagInfos.c_str());
// }

template<>
const HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::IPTagInfo *
HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::toIPTagInfo(const pat::Jet & jet, const std::string & tagInfos)
{
...
// -------------- toSVTagInfo ----------------
template<>
const HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::SVTagInfo *
HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::toSVTagInfo(const pat::Jet & jet, const std::string & tagInfos)
{
  return jet.tagInfoSecondaryVertex(tagInfos.c_str());
}

template<>
const HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::SVTagInfo *
HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::toSVTagInfo(const pat::Jet & jet, const std::string & tagInfos)
{
...
// -------------- setTracksPV ----------------
template<>
void HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::setTracksPV(const TrackRef & trackRef, const edm::Handle<reco::VertexCollection> & pvHandle, int & iPV, float & PVweight)
{
  setTracksPVBase(trackRef, pvHandle, iPV, PVweight);
}

template<>
void HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::setTracksPV(const TrackRef & trackRef, const edm::Handle<reco::VertexCollection> & pvHandle, int & iPV, float & PVweight)
{
  iPV = -1;
  PVweight = 0.;
...
// -------------- setTracksSV ----------------
template<>
void HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::setTracksSV(const TrackRef & trackRef, const SVTagInfo * svTagInfo, int & isFromSV, int & iSV, float & SVweight)
{
  isFromSV = 0;
  iSV = -1;
  SVweight = 0.;
...
template<typename IPTI,typename VTX>
void HLTTwoFileBTagAnalyzerT<IPTI,VTX>::setTracksSVCalo(const TrackRefCalo & trackRef, const reco::SecondaryVertexTagInfo * svTagInfo, int & isFromSV, int & iSV, float & SVweight)
{
  isFromSV = 0;
  iSV = -1;
  SVweight = 0.;
...
template<>
void HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::setTracksSV(const TrackRef & trackRef, const SVTagInfo * svTagInfo, int & isFromSV, int & iSV, float & SVweight)
{
  isFromSV = 0;
  iSV = -1;
  SVweight = 0.;
...
template<>
void HLTTwoFileBTagAnalyzerT<reco::TrackIPTagInfo,reco::Vertex>::vertexKinematicsAndChange(const Vertex & vertex, reco::TrackKinematics & vertexKinematics, Int_t & charge)
{
...
template<>
void HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::vertexKinematicsAndChange(const Vertex & vertex, reco::TrackKinematics & vertexKinematics, Int_t & charge)
{
  const std::vector<reco::CandidatePtr> & tracks = vertex.daughterPtrVector();
...
template<>
void HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate>::vertexKinematicsAndChangeCalo(const reco::Vertex & vertex, reco::TrackKinematics & vertexKinematics, Int_t & charge)
{
  Bool_t hasRefittedTracks = vertex.hasRefittedTracks();
...
// define specific instances of the templated HLTTwoFileBTagAnalyzer
typedef HLTTwoFileBTagAnalyzerT<reco::CandIPTagInfo,reco::VertexCompositePtrCandidate> HLTTwoFileBTagAnalyzer;
//define plugins
DEFINE_FWK_MODULE(HLTTwoFileBTagAnalyzer);
